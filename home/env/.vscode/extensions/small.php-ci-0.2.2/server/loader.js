...
class loader {
    ...
    allFun(document) {
        ...
        while ((match = this.re.fun.exec(content)) != null) {
            ...
            res.push({ name: match[1], kind: vscode_languageserver_1.SymbolKind.Method,
                location: { uri: uri,
                    range: {
    ...
    complete(textDocumentPosition, text) {
        ...
        if (isStatic) {
            ...
            if (claName.toLowerCase() == 'self') {
                ...
                if (!claInfo)
                    return res;
                claName = this.cache[claInfo.kind].get(claInfo.name).classData.name;
            }
        ...
        if (token.endsWith('$this')) {
            ...
            for (t of l) {
                if (t.indexOf('/') < 0)
                    res.push({
                        label: t, kind: vscode_languageserver_1.CompletionItemKind.Class, data: textDocumentPosition,
    ...
    _allWords(text, position, completeToken = true) {
        ...
        for (var index = 0, j = cha.length; index < j; index++) {
            if (cha[index] != '(' || !total.match(this.re.method))
                total += cha[index];
            else {
