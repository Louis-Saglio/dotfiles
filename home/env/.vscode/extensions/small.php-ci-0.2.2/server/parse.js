...
var wordsType;
(function (wordsType) {
    wordsType[wordsType["full"] = 0] = "full";
    wordsType[wordsType["half"] = 1] = "half";
    wordsType[wordsType["signature"] = 2] = "signature";
})(wordsType = exports.wordsType || (exports.wordsType = {}));
var completeType;
(function (completeType) {
    completeType[completeType["complete"] = 0] = "complete";
    completeType[completeType["less"] = 1] = "less";
    completeType[completeType["over"] = 2] = "over";
...
class parse {
    static parseFile(path) {
        ...
        while (match = this.re.class.exec(content)) {
            ...
            let arr = str.split('\n');
            var suff = arr.pop();
            if (suff.length > 0 && !suff.endsWith(' '))
                continue; //such as $class
            else
                suff = suff.trim();
    ...
    static parseConst(content, uri) {
        let classData = null, preClassD = null;
        let resConst = new Map();
    ...
    static _parseConst(content, classData) {
        let con = new Map();
        let match = null;
    ...
    static getWords(text, position, type = wordsType.full) {
    ...
    static cleanBracket(words, type = wordsType.full) {
            ...
            if (words[index] != '(')
                ...
                do {
                    ...
                    if (end < 0) {
                        ...
                        if (p.complete == completeType.less) {
                            tindex = end + 1;
                            continue;
                        }
                        else if (p.complete == completeType.over) {
        ...
        for (var index = 0, lim = words.length; index < lim; index++) {
            ...
            if (c == '\\') {
                ...
                if (c in this.pair) {
                    ...
                    return { param: total, complete: completeType.over };
                else
                    total += c;
            }
            else {
                ...
                else if (hardPair.indexOf(c) >= 0 && hardPair.indexOf(ignore) == -1) {
                    stack.push(c);
                    ignore = c;
                }
            }
