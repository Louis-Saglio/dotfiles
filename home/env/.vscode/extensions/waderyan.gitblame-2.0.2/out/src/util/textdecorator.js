...
class TextDecorator {
    static toTextView(commit) {
        if (blame_1.GitBlame.isBlankCommit(commit)) {
            return configuration_1.getProperty(configuration_1.Properties.StatusBarMessageNoCommit);
        }
        const normalizedCommitInfo = TextDecorator.normalizeCommitInfoTokens(commit);
        const messageFormat = configuration_1.getProperty(configuration_1.Properties.StatusBarMessageFormat);
